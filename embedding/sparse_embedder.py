...
class SparseEmbedder:
    ...
    def __update_vocabulary(self, tokens: list[str]): # Dùng để xây dựng vocabulary và document frequency
        ...
        """
        for token in set(tokens): # Dung set de tranh dem trung token trong document
            self.document_frequency[token] += 1 # Tang so lan xuat hien cua token trong cac document. VI du: token "nha" xuat hien trong 2 van ban thi document_frequency["nha"] = 2
